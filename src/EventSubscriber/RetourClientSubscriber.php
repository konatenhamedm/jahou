...
class RetourClientSubscriber implements EventSubscriberInterface
{
    private $em;
    private $workflow;
    ...
    public function handleValidation(TransitionEvent $event): void
    {
       ...
       // dd($entity);
        ...
        $lignes = $entity->getLigneRetourClients()->filter(function (LigneRetourClient $ligne) {
            return $ligne->getQuantite() != 0 ;
        })->toArray();

        $mouvement = new MouvementRetourCLient();
        ...
        $mouvement->setRetourClient($entity);
        ...
        foreach ($lignes as $ligne) {
            ...
            $ligne_mouvement->setRemise($ligne->getRemise());
    ...
    {
        return [
            'workflow.retour_client.transition.passer' => 'handleValidation',
