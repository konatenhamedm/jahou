...
class RetourFournisseurSubscriber implements EventSubscriberInterface
{
    private $em;
    private $workflow;
    ...
    public function handleValidation(TransitionEvent $event): void
    {
       ...
       // dd($entity);
        ...
        $lignes = $entity->getLigneRetourFournisseurs()->filter(function (LigneRetourFournisseur $ligne) {
            return $ligne->getQuantite() != 0 ;
        })->toArray();

        $mouvement = new MouvementRetourFournisseur();
        $mouvement->setReference($entity->getReference());
        $mouvement->setLibelle($entity->getLibelle());
        $mouvement->setDateMouvement(new \DateTime());
        $mouvement->setSens($this->sensRepository->findOneBy(array('libelle'=>'Sortie')));
        $mouvement->setRetourFournisseur($entity);
    ...
    {
        return [
            'workflow.retour_fournisseur.transition.passer' => 'handleValidation',
